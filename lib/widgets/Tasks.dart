...
class Tasks extends StatefulWidget {
  ...
  final bool
      readOnly; // پراپ جدید برای حالت فقط خواندن - اگر true باشد، دکمه‌های اکشن غیرفعال می‌شوند
  const Tasks(
      {super.key, required this.title, this.tasksList, this.readOnly = false});
...
class _TasksState extends State<Tasks> {
      ...
      if (widget.title == 'Daily Tasks') {
        ...
        List<Map<String, dynamic>> modifiedData = widget.tasksList!.map((item) {
          return {
            ...
            'type': "Questionnaire",
            'completed':
                item['Status'] == true || item['status'] == 'Done' ? 'Done' : ''
          };
        }).toList();
        modifiedData
            .sort((a, b) => a['id'].toString().compareTo(b['id'].toString()));
  ...
  Future<void> fetchQuestionary() async {
    ...
    final response = await _dio.post(widget.title == 'Daily Tasks'
        ? Endpoints.getTodaysTasks
        : Endpoints.getQuestionary);
          ...
          List<Map<String, dynamic>> modifiedData = jsonData.map((item) {
            if (item['Task_Type'] == 'Checkin') {
              return {
                ...
                'type': "Check-In",
                'completed':
                    item['Status'] == true ? 'Done' : '' // Add custom key
              };
            }
            if (item['Task_Type'] == 'Action') {
              if (item['Category'] == 'Diet' ||
        ...
        setState(() {
          List<Map<String, dynamic>> modifiedData = jsonData.map((item) {
            return {
              ...
              'title': item['title'],
              'type': "Questionnaire",
              'completed': item['Status'] == true || item['status'] == 'Done'
                  ? 'Done'
                  : '' // Add custom key
            };
          }).toList();
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          resolveCompletedTasksLength().toString() +
                              "/" +
                              resolveTasksLength().toString() +
                              " Completed",
        ...
        Container(
          child: tasks.isNotEmpty
              ? Container(
                  ...
                  child: widget.title == 'Daily Tasks'
                      ? Column(
                          children: List.generate(
                            5,
                            (index) => Padding(
                              padding: const EdgeInsets.only(
                                  bottom: 10), // Adds gap of 10 pixels
                              child: taskTypes[index] != 'Activity'
                                  ? TaskWrapper(
                                      typeName: taskTypes[index],
                                      tasks: tasks
                                          .where((task) =>
                                              task['type'] == taskTypes[index])
                                          .toList(),
                      ...
                      : Padding(
                          padding: const EdgeInsets.only(
                              bottom: 10), // Adds gap of 10 pixels
                          child: TaskWrapper(
                            typeName: taskTypes[5],
                            tasks: tasks
                                .where((task) => task['type'] == taskTypes[5])
                                .toList(),
                            readOnly: widget.readOnly,
                            onTaskCompletionChanged: _onTaskCompletionChanged,
                          ),
                        ),
                )
              : EmptyBox(
                  iconPath: widget.title == 'Daily Tasks'
                      ? "assets/calendar-2.svg"
                      : "assets/favrit.svg",
                  text: widget.title == 'Daily Tasks'
                      ? "No tasks for today yet"
                      : "No flexible tasks assigned",
                  title: widget.title,
                ),
        ),
      ],
